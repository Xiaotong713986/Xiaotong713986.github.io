...
const parseGoTemplate = (text, parsers, options) => {
    ...
    const regex = /{{(?<startdelimiter>-|<|%|\/\*)?\s*(?<statement>(?<keyword>if|range|block|with|define|end|else|prettier-ignore-start|prettier-ignore-end)?[\s\S]*?)\s*(?<endDelimiter>-|>|%|\*\/)?}}|(?<unformattableScript>\s*<(script)((?!<\/script)[\s\S])*?{{[\s\S]*?<\/(script)>)|(?<unformattableStyle>\s*<(style)((?!<\/style)[\s\S])*?{{[\s\S]*?<\/(style)>)/g;
    const blocks = [];
    const root = {
        type: "root",
        content: text,
